...
The `com.enterprise.batch.spring` package bridges the SQL DSL with Spring Batch's `ItemReader` / `ItemWriter` infrastructure. The key principle: **providers build fresh queries on every call**, ensuring thread safety and parameter isolation.
